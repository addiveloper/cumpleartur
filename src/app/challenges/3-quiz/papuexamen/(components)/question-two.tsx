...
import { QuestionTwoSchema } from "@/schemas/papuexamen";
...
export const QuestionTwo = ({ setQuestion, setProgress }: QuestionTwoProps) => {
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState("");

  const form = useForm<z.infer<typeof QuestionTwoSchema>>({
    resolver: zodResolver(QuestionTwoSchema),
  });

  const handleSubmit = async (values: z.infer<typeof QuestionTwoSchema>) => {
    setIsLoading(true);
    setError("");
    try {
      ...
      <div className="flex w-full justify-between">
        <div>
          <p className="dark:text-neutral-400 text-neutral-600">
            Segunda papu-pregunta :v
          </p>
          <h2 className="font-bold text-3xl pb-16">¿Qué hay en Papulandia?</h2>
        </div>
